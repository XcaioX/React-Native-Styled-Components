...
export const AuthProvider: React.FC = ({ children }) => {
  ...
  useEffect(() => {
    const loadStorage = async () => {
      const storageCredentials = await AsyncStorage.multiGet([
        '@Auth:user',
        '@Auth:token',
      ]);
  ...
  const signOut = useCallback(async (): Promise<void> => {
    AsyncStorage.multiRemove(['@Auth:user', '@Auth:token']).then(() =>
      setData(null),
    );
  ...
  return (
    <AuthContext.Provider
      value={{ authData: data, loading, signIn, signOut, signUp }}>
      {children}
    </AuthContext.Provider>
  );
};
